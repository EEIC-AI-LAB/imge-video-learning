...
def train_lora(
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            pipeline.save_pretrained(f"./trained_end/fine_tuned_sd_lora_best")
            print(f"Best model saved at epoch {epoch} with val_loss: {val_loss:.6f}")
            
        pipeline.save_pretrained(f"./trained_end/fine_tuned_stable_diffusion_lora_{epoch}")
...
if __name__ == "__main__":
    ...
    pipeline = StableDiffusionImg2ImgPipeline.from_pretrained(
        "./fine_tuned_stable_diffusion_lora", torch_dtype=torch.float16).to("cuda")
    ...
    pipeline.save_pretrained(f"./trained_end/fine_tuned_stable_diffusion_lora_honban")
